...
!
  type ErrorHandle
    ...
    type(vector_character)                  :: message
    integer                                 :: tnum = -1, enum = 0, base = 0
  contains
    procedure :: init          => ErrInit
    procedure :: warn          => ErrWarn
    procedure :: check         => ErrCheck
    procedure :: punch         => ErrPunch
    procedure :: punchabort    => ErrPunchAbort
    final     :: ErrDestractor
  end type ErrorHandle
!
  type OptNode
    type(vector_character)     :: s, arg, def
...
!
  type Optparse
    ...
    type(vector_character)     :: CommandArg,FixedArg
    type(vector_character)     :: helpargs,help,Metavar
    character(:),allocatable   :: ScriptName,VersionNumber
    type(optnode),allocatable  :: opt(:)
    type(vector_character)     :: error,warn

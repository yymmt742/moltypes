...
!
  call compute(natm,nres,ntrj,ratm,rres,crd,refcrd,box,am,rm,&
              &arg%optargs('-r',1),arg%optargs('-a',1),arg%optargs('-g',1))
!
  time2 = omp_get_wtime()
...
!
    do i=1,ninp
      ...
      call trj%load(inp(i),stride=interval)
    enddo
!
    ntrj = trj%n_frames()
!
...
!
  subroutine compute(natm,nres,ntrj,ratm,rres,crd,refcrd,box,am,rm,r,a,g)
  ...
  character(*),intent(in)        :: r,a,g
  double precision               :: rcpbox(3,ntrj)
  double precision               :: rc(ntrj,nres,0:rres),ac(ntrj,nres,0:rres),gc(ntrj,nres,0:rres)
  ...
  integer                        :: i,j,progres,w1,w2,wcnt
    nr = -1.d0 ; rc = 0.d0 ; ac = 0.d0 ; gc = 0.d0
    seq = [(i,i=1,maxval([nres,rres],1))]
!
    do i=1,ntrj
      rcpbox(:,i) = reciprocalBox(box(:,i))
    enddo
...
!
    do i=1,ntrj
      nlen = ntrj - i + 1
      call get_network(natm,nres,ratm,rres,crd(:,:,:,i),refcrd(:,:,:,i),box(:,i),rcpbox(:,i),am,rm,bonds,label)
      do j=1,nres+rres
...
!
        ...
        call compute_timecorr(natm,nres,nclu,ratm,rres,rclu,nsum,nlen,bonds(j)%size(),bonds(j)%lookup(),&
                             &crd(:,:,:,i:),refcrd(:,:,:,i:),box(:,i:),rcpbox(:,i:),&
                             &rc(1:nlen,nclu,rclu),ac(1:nlen,nclu,rclu),gc(1:nlen,nclu,rclu),&
                             &nr(1:nlen,nclu,rclu),ns,rs)
      enddo
!
...
!
    ...
    call output_timecorr(natm,nres,rres,ntrj,trim(r),trim(a),trim(g),rc,ac,gc,nr)
  end subroutine compute
!
  subroutine get_network(natm,nres,ratm,rres,crd,refcrd,box,rcpbox,am,rm,bonds,label)
  integer,intent(in)               :: natm,nres,ratm,rres
  double precision,intent(in)      :: crd(3,natm,nres),refcrd(3,ratm,rres),box(3),rcpbox(3)
  logical,intent(in)               :: am(3,natm),rm(3,ratm)
...
!
  subroutine catchSnap(x,natm,nres,ratm,rres,seq,c0)
  character(*),intent(in)        :: x
  integer,intent(in)             :: natm,nres,ratm,rres,seq
  double precision,intent(in)    :: c0(3,natm*nres+ratm*rres)
  type(fileio)                   :: ncout
  character(21)                  :: serial
    write(serial,'(a,i3.3,a,i6.6,11x)')'_',nres,'_',seq
...
!
  subroutine compute_timecorr(natm,nres,nclu,ratm,rres,rclu,nsum,ntrj,nbond,bond,crd,refcrd,box,rcpbox,rc,ac,gc,nr,ns,rs)
  integer,intent(in)             :: natm,nres,nclu,ratm,rres,rclu,nsum,ntrj,nbond,bond(nbond)
  double precision,intent(in)    :: crd(3,natm,nres,ntrj),refcrd(3,ratm,rres,ntrj),box(3,ntrj),rcpbox(3,ntrj)
  double precision,intent(inout) :: rc(ntrj),ac(ntrj),gc(ntrj),nr(ntrj)
  logical,intent(in)             :: ns(3,natm,nres),rs(3,ratm,rres)
  double precision               :: c0(3,nsum),c1(3,nsum),rot(3,3)
  double precision               :: r(3),rms,rog,aco,rcpn,rcpac
  logical                        :: attach(nbond),search(nbond)
  integer                        :: i,j,k,ll,n3
    n3 = 3*natm
    c0 = unwrap(natm,nclu,ratm,rclu,pack(crd(:,:,:,1),ns),pack(refcrd(:,:,:,1),rs),box(:,1),rcpbox(:,1))
!
    do i=1,ntrj
      c1 = unwrap(natm,nclu,ratm,rclu,pack(crd(:,:,:,i),ns),pack(refcrd(:,:,:,i),rs),box(:,i),rcpbox(:,i))
!
      search = .FALSE.
      do j=1,nbond
        if(search(j))CYCLE
        ...
        do k=j+1,nbond
          ...
          attach(k) = checkAttach(n3,n3,c1(:,(j-1)*natm+1:j*natm),c1(:,(k-1)*natm+1:k*natm),box(:,i),rcpbox(:,i))
        enddo
        if(.not.ANY(attach))RETURN
        search = search.or.attach
      enddo
!
...
!
  subroutine output_timecorr(natm,nres,rres,ntrj,r,a,g,rc,ac,gc,nr)
  integer,intent(in)             :: natm,nres,rres,ntrj
  character(*),intent(in)        :: r,a,g
  double precision,intent(in)    :: rc(ntrj,nres,0:rres)
